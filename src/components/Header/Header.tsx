...
export default function Header() {
  ...
  useClickOutSide(menuMobileRef, () => setIsActiveMenu(false))
      ...
      <header className='sticky top-0 z-[1000]'>
        <div className='blog-container '>
          <div className='blog-card h-[65px] lg:h-[80px] px-4 md:px-8 flex justify-between items-center relative'>
            ...
            >
              <span
                className={`h-[3px] w-full bg-black dark:bg-white ease-in-out duration-500 absolute top-[10%] left-0 ${
                  isActiveMenu ? 'rotate-45 translate-y-[9px] ' : ''
                }`}
              ></span>
              <span
                className={`h-[3px] w-full bg-black dark:bg-white ease-in-out duration-500 absolute -translate-y-1/2 top-[50%] ${
                  isActiveMenu ? '-translate-x-[8px] opacity-0' : ''
                }`}
              ></span>
              <span
                className={`h-[3px] w-full bg-black dark:bg-white ease-in-out duration-500 absolute bottom-[10%] left-0 ${
                ...
                {menuHeader.map((item, index) => (
                  <li
                    ...
                    className={classNames('relative text-center', {
                      ...
                      'text-gray-800 dark:text-gray-100': pathname !== item.path
                    })}
                  >
